...
function htmlblock(state, startLine, endLine, silent) {
  ...
  if (!isLetter$1(state.src.charCodeAt(pos + 1))) { return false; }

  const match = state.src.slice(pos, max).match(HTML_TAG_OPEN_RE);
  // Is the open tag self-closing?
  const self_closing = match && (html_voids[match[1]] || match[0].match(/<[A-Za-z][A-Za-z0-9]*.*\s*\/>/));

  if (!match || self_closing) { return false; }
...
function paragraph(state, startLine/*, endLine*/) {
  ...
  const html_match = content.match(HTML_TAG_OPEN_RE$1);
  const is_block = html_match && html_blocks[html_match[1]];
...
function htmltag(state, silent) {
  ...
  // Is the open tag self-closing?
  self_closing = html_voids[tag_name] || match_open[0].match(/<[A-Za-z][A-Za-z0-9]*.*\s*\/>/);
